...
export default function Shares() {
  ...
  const [filtered, setFiltered] = useState<ShareItem[]>([]);
  ...
  useEffect(() => {
    let list = [...items];
    if (fUser) list = list.filter(s => s.user_name.includes(fUser));
    if (fStatus) list = list.filter(s => s.status === fStatus);
    if (fDateFrom) list = list.filter(s => s.created_at >= fDateFrom);
    if (fDateTo) list = list.filter(s => s.created_at <= fDateTo + ' 23:59:59');
    if (fKeyword) list = list.filter(s => s.token.includes(fKeyword) || s.user_name.includes(fKeyword));
    setFiltered(list);
  }, [items, fUser, fStatus, fDateFrom, fDateTo, fKeyword]);
  ...
  const execBatchDelete = async () => {
    try { await api.post('/admin/shares/batch-delete', { ids: Array.from(selected) }); toast.success('已删除'); setSelected(new Set()); load(); }
  ...
  const batchStatus = async (status: string) => {
    try { await api.post('/admin/shares/batch-status', { ids: Array.from(selected), status }); toast.success(`已${status === 'active' ? '恢复' : '失效'} ${selected.size} 条`); setSelected(new Set()); load(); }
    catch (err: any) { toast.error(err.response?.data?.message || '操作失败'); }
  };
  ...
  const handleExportExcel = () => {
    if (filtered.length === 0) { toast.error('无数据可导出'); return; }
    try {
      const ws = XLSX.utils.json_to_sheet(filtered.map(s => ({
        ...
        '浏览次数': s.visit_count,
        '下载次数': s.download_count,
        '最后访问': s.last_access_at || '-',
        'IP地址': s.ip_address || '-',
        '有效期': s.expires_at ? new Date(s.expires_at).toLocaleString('zh-CN') : '-',
        '分享时间': s.created_at?.split('.')[0].replace('T', ' ') || '-',
        '链接': `${window.location.origin}/shared/${s.token}`,
      })));
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, '分享台账');
  ...
  return (
    <div>
      ...
      {/* Filter bar */}
      <div className="border border-[var(--border)] rounded-2xl shadow-sm bg-[var(--card)] mb-4">
        <div className="p-4 space-y-3">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            <input placeholder="分享人" value={fUser} onChange={e => setFUser(e.target.value)}
              className="px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--background)] text-xs text-[var(--foreground)] outline-none focus:border-[var(--primary)]" />
            <select value={fStatus} onChange={e => setFStatus(e.target.value)}
              className="px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--background)] text-xs text-[var(--foreground)] outline-none focus:border-[var(--primary)]">
              <option value="">全部状态</option><option value="active">有效</option><option value="expired">已过期</option><option value="disabled">已失效</option>
            </select>
            <input type="date" value={fDateFrom} onChange={e => setFDateFrom(e.target.value)}
              className="px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--background)] text-xs text-[var(--foreground)] outline-none focus:border-[var(--primary)]" />
            <input type="date" value={fDateTo} onChange={e => setFDateTo(e.target.value)}
              className="px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--background)] text-xs text-[var(--foreground)] outline-none focus:border-[var(--primary)]" />
            <input placeholder="搜索链接/分享人" value={fKeyword} onChange={e => setFKeyword(e.target.value)}
              className="px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--background)] text-xs text-[var(--foreground)] outline-none focus:border-[var(--primary)]" />
          </div>
          <div className="flex items-center gap-2 text-xs">
            <span className="text-[var(--muted-foreground)]">共 {filtered.length} 条</span>
            <button onClick={() => { setFUser(''); setFStatus(''); setFDateFrom(''); setFDateTo(''); setFKeyword(''); }}
              className="text-[var(--primary)] hover:underline">重置筛选</button>
          </div>
        </div>
      </div>
      ...
      {filtered.length > 0 && (
        <div className="flex items-center gap-2 mb-3 flex-wrap">
          <label className="flex items-center gap-1.5 text-xs text-[var(--muted-foreground)] cursor-pointer">
            <input type="checkbox" checked={allSelected} onChange={() => { if (allSelected) setSelected(new Set()); else setSelected(new Set(filtered.map(s => s.id))); }}
              className="w-4 h-4 rounded border-[var(--border)] accent-[var(--primary)]" />全选
          </label>
          {selected.size > 0 && (
            <>
              <span className="text-xs text-[var(--muted-foreground)]">已选 {selected.size} 项</span>
              <button onClick={() => batchStatus('disabled')} className="px-3 py-1 text-xs rounded-lg bg-[var(--muted)] text-[var(--muted-foreground)] font-medium">批量失效</button>
              <button onClick={() => setConfirmBatchDelete(true)} className="px-3 py-1 text-xs rounded-lg bg-[var(--destructive)]/10 text-[var(--destructive)] font-medium">批量删除</button>
              <button onClick={handleExportExcel} className="px-3 py-1 text-xs rounded-lg bg-[var(--primary)] text-[var(--primary-foreground)] font-medium">导出台账</button>
            </>
          )}
          {selected.size === 0 && (
            <button onClick={handleExportExcel} className="px-3 py-1 text-xs rounded-lg bg-[var(--primary)] text-[var(--primary-foreground)] font-medium">导出台账</button>
        ...
        <div className="overflow-x-auto" style={{ overflowY: 'visible', maxHeight: 'calc(100vh - 320px)' }}>
          ...
          : (
            <table className="app-table">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={13} className="text-center py-8 text-[var(--muted-foreground)]">暂无分享记录</td></tr>
                ) : filtered.map((s) => {
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <div className="flex-1 relative">
                <input type="number" min={1} max={365} value={renewDays} onChange={e => setRenewDays(parseInt(e.target.value) || 7)}
                  className={`w-full py-2 text-xs text-center rounded-lg font-medium outline-none transition-all ${![7, 30].includes(renewDays) ? 'bg-[var(--primary)] text-[var(--primary-foreground)]' : 'bg-[var(--muted)] text-[var(--muted-foreground)]'}`}
                  placeholder="自定义" />
      ...
      <ConfirmModal open={confirmBatchDelete} title="批量删除" message={`确定要删除选中的 ${selected.size} 条分享记录？`} onConfirm={execBatchDelete} onCancel={() => setConfirmBatchDelete(false)} danger />

      {previewItem && (
        <PreviewModal
          items={[{ url: previewItem.url, original_name: previewItem.original_name, type: previewItem.type, thumbnail_url: previewItem.thumbnail_url || '' }]}
          index={0} onClose={() => setPreviewItem(null)}
        />
      )}
    </div>
  );
