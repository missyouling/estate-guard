...
interface EvidenceItem {
  id: number; record_no: number; type: string; category_name: string;
  original_name: string; url: string; thumbnail_url?: string; address?: string;
  user_name?: string; storage_location?: string; watermark_applied: boolean;
  uploaded_at: string; size_bytes: number; file_hash?: string;
}
...
export default function Export() {
  ...
  const doPrint = () => {
    ...
    if (!printReady) { printPendingRef.current = true; return; }
  ...
  useEffect(() => {
    if (focusId && items.length > 0 && !loading && !focusHandledRef.current) {
      ...
      if (idx >= 0) { setPreviewItem(items[idx]); setPreviewIndex(idx); }
  ...
  useEffect(() => {
    if (!focusHandledRef.current) { setPreviewItem(null); setPreviewIndex(0); }
  }, [items]);
  ...
  const handleSearch = async () => {
    setLoading(true); setSelected([]); setSelectMode(false);
    try {
      const params: any = {};
      ...
      params.page = 1; params.limit = 10000;
  ...
  const handlePrint = () => {
    const toPrint = selected.length > 0 ? selectedItems : items;
...
${toPrint.map(it => `<tr><td>NO.${it.record_no}</td><td>${{image:'图片',video:'视频',audio:'音频',document:'文件'}[it.type]||it.type}</td><td>${it.category_name||'-'}</td><td>${it.original_name}</td><td>${it.address||'-'}</td><td>${it.user_name||'-'}</td><td>${it.uploaded_at}</td></tr>`).join('')}
  ...
  const handlePreview = async (item: EvidenceItem, idx: number) => {
    ...
    if (item.type === 'document') {
      ...
      setPreviewItem(item); setPreviewIndex(i >= 0 ? i : idx); return;
    }
  ...
  const handleBatchDownload = async (ids?: number[]) => {
    const toDl = ids && ids.length > 0 ? items.filter(it => ids.includes(it.id)) : items;
    ...
    try {
      ...
      if (res.data.type === 'application/json') {
        ...
        window.URL.revokeObjectURL(url); toast.success('下载完成');
      }
  ...
  const execDelete = async () => {
    ...
    const item = confirmDelete; setConfirmDelete(null);
    try {
  ...
  const handleExportExcel = () => {
    ...
    try {
      const ws = XLSX.utils.json_to_sheet(items.map(it => ({
        '证据编号': `NO.${it.record_no}`, '分类': it.category_name || '-', '文件名': it.original_name,
        '上传人': it.user_name || '-', '上传时间': it.uploaded_at || '-',
        '存证状态': evidenceStatus(it.file_hash).label, 'SHA-256 哈希': it.file_hash || '-',
      })));
      const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, '存证清单');
...
.cert{max-width:700px;margin:0 auto;border:2px solid #E5E5EA;border-radius:16px;padding:48px}h1{text-align:center;font-size:22px;margin:0 0 4px}h2{text-align:center;font-size:13px;color:#86868B;font-weight:400;margin:0 0 32px;padding-bottom:24px;border-bottom:1px solid #E5E5EA}
...
.fingerprint{background:#f5f5f7;border-radius:12px;padding:16px;margin-top:24px;font-size:12px}.fingerprint .label{color:#86868B;font-size:11px;margin-bottom:8px}.fingerprint .hash{font-family:ui-monospace,SFMono-Regular,monospace;font-size:11px;word-break:break-all;color:#1D1D1F;line-height:1.6}
...
<dl class="info"><dt>证据编号</dt><dd>NO.${item.record_no}</dd><dt>文件名称</dt><dd>${item.original_name}</dd>
<dt>文件大小</dt><dd>${formatBytes(item.size_bytes)}</dd><dt>文件类型</dt><dd>${typeLabel(item.type)}</dd>
<dt>文件分类</dt><dd>${item.category_name || '-'}</dd><dt>上传人</dt><dd>${item.user_name || '-'}</dd>
<dt>上传时间</dt><dd>${item.uploaded_at}</dd><dt>存储位置</dt><dd>${item.storage_location || '本地存储'}</dd>
${item.file_hash ? `<div class="fingerprint"><div class="label">数字指纹 (SHA-256)</div><div class="hash">${item.file_hash}</div></div>` : ''}
</dl><div class="footer">本凭证由系统自动生成，用于证明文件在存证时间点后的完整性<br>凭证生成时间：${new Date().toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' })}</div>
</div></body></html>`;
    const a = document.createElement('a');
    a.href = 'data:text/html;charset=utf-8,' + encodeURIComponent(html);
    a.download = `存证凭证_NO.${item.record_no}.html`; a.click();
    toast.success('凭证已导出，打开后可使用浏览器打印为PDF');
  };

  const Svg = {
    shieldCheck: <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M12 2l7 4v5c0 5-3.5 9.7-7 11-3.5-1.3-7-6-7-11V6l7-4z" fill="currentColor" opacity="0.15"/><path d="M12 2l7 4v5c0 5-3.5 9.7-7 11-3.5-1.3-7-6-7-11V6l7-4z" stroke="currentColor" strokeWidth="1.5" fill="none"/><path d="M9 12l2 2 4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>,
    shieldAlert: <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" fill="currentColor" opacity="0.15"/><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="1.5" fill="none"/><path d="M12 8v4M12 16h0" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>,
    detail: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M9 12h6M12 9v6"/></svg>,
    verify: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}><path d="M12 2l7 4v5c0 5-3.5 9.7-7 11-3.5-1.3-7-6-7-11V6l7-4z"/><path d="M9 12l2 2 4-4" strokeLinecap="round" strokeLinejoin="round"/></svg>,
    ...
    download: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3"/></svg>,
  };

  const columns: Column<EvidenceItem>[] = [
    { key: 'record_no', label: '编号', width: 100, fixed: 'left',
      render: (v, row) => {
        const st = evidenceStatus(row.file_hash);
        return <div className="flex items-center gap-1.5">
          <span title={st.tip} style={{ color: st.color, flexShrink: 0 }}>{st.icon === 'shield-check' ? Svg.shieldCheck : Svg.shieldAlert}</span>
          <span className="font-mono text-xs">NO.{v}</span>
        </div>;
      } },
    { key: 'type', label: '类型', width: 50, render: (v) => <span className="text-xs">{typeLabel(v)}</span> },
    { key: 'category_name', label: '分类', width: 80, render: (v) => <span className="text-xs text-[var(--muted-foreground)]">{v || '-'}</span> },
    { key: 'original_name', label: '文件名', width: 200, render: (v) => <span className="text-xs block truncate" title={v}>{v}</span> },
    { key: 'address', label: '情况说明', width: 150, render: (v) => <span className="text-xs text-[var(--muted-foreground)] block truncate" title={v || '-'}>{v || '-'}</span> },
    { key: 'user_name', label: '上传人', width: 80, render: (v) => <span className="text-xs">{v || '-'}</span> },
    { key: 'storage_location', label: '存储位置', width: 100, render: (v) => <span className="text-xs text-[var(--muted-foreground)]">{v || '本地存储'}</span> },
    { key: 'uploaded_at', label: '上传时间', width: 150, render: (v) => <span className="text-xs text-[var(--muted-foreground)]">{v}</span> },
    { key: 'size_bytes', label: '大小', width: 70, render: (v) => <span className="text-xs text-[var(--muted-foreground)]">{formatBytes(v)}</span> },
    { key: 'actions', label: '操作', width: 120, fixed: 'right',
      render: (_, row) => <div className="flex items-center gap-1">
        <button onClick={e => { e.stopPropagation(); setDetailItem(row); }} className="text-[var(--muted-foreground)] hover:text-[var(--primary)] p-1" title="存证详情">{Svg.detail}</button>
        <button onClick={e => { e.stopPropagation(); setVerifyItem(row); }} className="text-[var(--muted-foreground)] hover:text-[var(--primary)] p-1" title="文件验真">{Svg.verify}</button>
        <button onClick={e => { e.stopPropagation(); handleDownload(row); }} className="text-[var(--muted-foreground)] hover:text-[var(--primary)] p-1" title="下载">{Svg.download}</button>
        <button onClick={e => { e.stopPropagation(); setConfirmDelete(row); }} className="text-[var(--muted-foreground)] hover:text-[var(--destructive)] p-1" title="删除">{Svg.trash}</button>
      </div> },
  ];

  const batchActions: BatchAction[] | undefined = selectMode ? [
    { key: 'download', label: `下载 (${selected.length})`, onClick: (ids) => handleBatchDownload(ids) },
  ] : undefined;
  ...
  return (
    <div>
      ...
      {/* Filter bar */}
      <div className="border border-[var(--border)] rounded-2xl overflow-hidden shadow-sm bg-[var(--card)] mb-5">
        <div className="p-5 pb-3 space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            <div><label className="text-xs font-semibold text-[var(--muted-foreground)] mb-1 block">分类</label>
            ...
            <div><label className="text-xs font-semibold text-[var(--muted-foreground)] mb-1 block">类型</label>
            ...
            <div><label className="text-xs font-semibold text-[var(--muted-foreground)] mb-1 block">开始日期</label><input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--background)] text-sm" /></div>
            <div><label className="text-xs font-semibold text-[var(--muted-foreground)] mb-1 block">结束日期</label><input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--background)] text-sm" /></div>
          ...
          {items.length > 0 && (
            <div className="flex items-center gap-2 pt-2 border-t border-[var(--border)] flex-wrap">
              ...
              <button onClick={() => { setSelectMode(!selectMode); setSelected([]); }}
                className={`px-3 py-1 text-xs rounded-lg font-medium ${selectMode ? 'bg-[var(--primary)] text-[var(--primary-foreground)]' : 'bg-[var(--muted)] text-[var(--muted-foreground)]'}`}>
                {selectMode ? '取消选择' : '选择'}</button>
              {selectMode && <span className="text-xs text-[var(--muted-foreground)]">已选 {selected.length} 项</span>}
              <div className="flex-1" />
              <button onClick={() => handleBatchDownload(undefined)} className="px-3 py-1 text-xs rounded-lg bg-[var(--muted)] text-[var(--muted-foreground)] font-medium">
                {selected.length > 0 ? `下载 (${selected.length})` : '下载全部'}</button>
              <button onClick={() => setShowShare(true)}
                className="px-3 py-1 text-xs rounded-lg bg-[var(--primary)] text-[var(--primary-foreground)] font-medium">分享 {selected.length > 0 ? `(${selected.length})` : ''}</button>
              <button onClick={() => { handlePrint(); const toPrint = selected.length > 0 ? selectedItems : items; /* reuse same logic */ }}
                className="px-3 py-1 text-xs rounded-lg bg-[var(--muted)] text-[var(--muted-foreground)] font-medium">打印预览 {selected.length > 0 ? `(${selected.length})` : ''}</button>
              <button onClick={handleExportExcel}
                className="px-3 py-1 text-xs rounded-lg bg-[var(--primary)] text-[var(--primary-foreground)] font-medium">导出存证清单</button>
            </div>
          )}
        </div>
      </div>
      ...
      <DataTable<EvidenceItem>
        ...
        data={items}
        loading={loading}
        selection={selectMode ? 'multi' : 'none'}
        selectedIds={selected}
        onSelectionChange={setSelected}
        onRowDoubleClick={(row) => handlePreview(row, items.indexOf(row))}
        batchActions={batchActions}
        columnConfigKey="export_visibleCols"
        minWidth={1100}
        emptyText="暂无证据数据"
      />
      ...
      {previewItem && (
        <PreviewModal
          items={(previewItems.length > 0 ? previewItems : items).map(it => ({
            url: it.url, original_name: it.original_name, type: it.type,
      ...
      {showShare && (() => {
        const toShare = (selected.length > 0 ? selectedItems : items);
        return <ShareModal mediaIds={toShare.map(it => it.id)} mediaItems={toShare as unknown as Media[]} onClose={() => setShowShare(false)} />;
      })()}
          ...
          <div className="bg-[var(--background)] border border-[var(--border)] rounded-2xl shadow-2xl w-full max-w-lg flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-3 border-b border-[var(--border)]">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setDetailItem(null)} className="p-1 hover:bg-[var(--accent)] rounded-lg"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M18 6L6 18M6 6l12 12"/></svg></button>
              </div>
            </div>
            <div className="p-5 space-y-4 overflow-y-auto max-h-[70vh]">
              ...
              {detailItem.file_hash && (
                <div className="bg-[var(--muted)] rounded-xl p-4 space-y-2">
                  <div className="flex items-center justify-between"><span className="text-xs font-semibold text-[var(--foreground)]">数字指纹 (SHA-256)</span>
                    <button onClick={() => { navigator.clipboard.writeText(detailItem.file_hash || ''); toast.success('已复制'); }} className="text-[10px] text-[var(--primary)] font-medium hover:underline">一键复制</button></div>
              ...
              <div className="bg-[var(--muted)] rounded-xl p-4 space-y-2">
                ...
                <div className="space-y-1.5 text-xs">
                  <div className="flex items-center gap-2"><span className="text-[var(--muted-foreground)]">水印叠加：</span>
                    {detailItem.watermark_applied ? <span className="flex items-center gap-1 text-[#34C759]"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}><path d="M20 6L9 17l-5-5"/></svg>已叠加</span> : <span className="text-[var(--muted-foreground)]">未叠加</span>}</div>
                  <div className="flex items-center gap-2"><span className="text-[var(--muted-foreground)]">元数据嵌入：</span>
                    {['video', 'audio'].includes(detailItem.type) ? <span className="flex items-center gap-1 text-[#34C759]"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}><path d="M20 6L9 17l-5-5"/></svg>已嵌入</span>
                    : detailItem.type === 'image' ? <span className="text-[var(--muted-foreground)]">不适用（图片水印替代）</span>
                    : <span className="text-[var(--muted-foreground)]">不支持此格式</span>}</div>
                </div>
              </div>
            </div>
          </div>
      ...
      {verifyItem && <FileVerifyModal item={verifyItem} onClose={() => setVerifyItem(null)} />}
    </div>
  );
}
...
function sha256Hex(data: ArrayBuffer): string {
  ...
  for (let offset = 0; offset < padLen; offset += 64) {
    ...
    for (let t = 16; t < 64; t++) W[t] = (om1(W[t - 2]) + W[t - 7] + om0(W[t - 15]) + W[t - 16]) >>> 0;
...
function FileVerifyModal({ item, onClose }: { item: EvidenceItem; onClose: () => void }) {
  ...
  const computeHash = async () => {
    ...
    setComputing(true); setProgress(0);
    try {
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-[var(--background)] border border-[var(--border)] rounded-2xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-3 border-b border-[var(--border)]">
          ...
          <button onClick={onClose} className="p-1 hover:bg-[var(--accent)] rounded-lg"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M18 6L6 18M6 6l12 12"/></svg></button>
        </div>
        <div className="p-5 space-y-4">
          ...
          <div><label className="block text-xs text-[var(--muted-foreground)] mb-1.5">选择本地文件</label>
            <input type="file" onChange={handleFileChange}
              className="w-full text-xs text-[var(--foreground)] file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-medium file:bg-[var(--primary)] file:text-[var(--primary-foreground)] hover:file:opacity-80 cursor-pointer" /></div>
          {selectedFile && !computing && !result && (
            <button onClick={computeHash} className="w-full py-2 rounded-lg bg-[var(--primary)] text-[var(--primary-foreground)] text-xs font-medium">开始验真</button>)}
          {(computing || progress > 0) && (
            <div className="space-y-1.5">
              ...
              <div className="w-full h-2 bg-[var(--muted)] rounded-full overflow-hidden"><div className="h-full rounded-full transition-all duration-300" style={{ width: `${progress}%`, backgroundColor: result?.match ? '#34C759' : result && !result.match ? '#FF3B30' : '#007AFF' }} /></div>
            </div>)}
          {result && (
            <div className={`rounded-xl p-4 ${result.match ? 'bg-[#34C759]/10' : 'bg-[#FF3B30]/10'}`}>
              <div className="flex items-center gap-2 mb-2">
                {result.match ? <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#34C759" strokeWidth={2.5}><path d="M20 6L9 17l-5-5"/></svg>
                  : <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#FF3B30" strokeWidth={2.5}><path d="M18 6L6 18M6 6l12 12"/></svg>}
                <span className={`text-sm font-semibold ${result.match ? 'text-[#34C759]' : 'text-[#FF3B30]'}`}>{result.match ? '文件与原始存证一致，未被篡改' : '文件内容已被修改，与原始存证不符'}</span>
              </div>
